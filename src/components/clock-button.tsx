...
import { completeTimeEntryAction } from "@/app/(agent)/actions";
import { clearPending, readPending, writePending, type PendingClockAction } from "@/lib/clock-queue";

// Terminer une vacation sans réseau (sous-sol, parking) ne doit pas perdre
// le clic. Appelle la Server Action directement (pas de <form> : supporté
// nativement pour un appel programmatique depuis un client component) ;
// une vraie panne réseau échoue avant même d'atteindre le serveur — mise
// en attente locale (src/lib/clock-queue.ts), rejouée au retour du
// réseau. Une erreur serveur réelle (pas un souci de connexion) n'est PAS
// avalée : silencieusement la retenter en boucle masquerait un vrai bug
// plutôt que de le corriger.
export function ClockButton({ targetId, className }: { targetId: string; className: string }) {
  ...
  async function run(action: PendingClockAction) {
    // Optimiste : la Server Action se termine toujours par redirect()
    // (succès comme échec applicatif), qui interrompt la fonction en
    // levant une erreur interne Next — tout code placé après le await, y
    // compris setPending(null), n'est donc JAMAIS atteint sur le chemin
    // de succès. On vide la file et l'état "en attente" avant la
    // tentative ; le catch les réécrit si elle échoue vraiment hors ligne.
    clearPending();
    setPending(null);
    try {
      await completeTimeEntryAction(action.targetId, action.note);
      router.refresh();
    } catch (error) {
  ...
  function handleClick() {
    ...
    const action: PendingClockAction = { targetId, note, queuedAt: new Date().toISOString() };
