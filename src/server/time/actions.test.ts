...
import { completeTimeEntry } from "./actions";
import { listTodayShiftsForAgent } from "./queries";
...
describe("droits Pointage — seul un AGENT pointe, seuls ADMIN/PLANNER valident", () => {
  it("completeTimeEntry rejette un ADMIN (pas un agent)", async () => {
    await expect(completeTimeEntry(admin, "any-shift")).rejects.toBeInstanceOf(ForbiddenError);
  });
